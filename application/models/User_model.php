...
class user_model extends CI_Model
{
    ...
    public function validate_user($data) {
        ...
        $stmt = $this->ceh->where($where)->limit(1)->get('SA_USERS');
    ...
    public function check_exist_login_user($UserID){
        $count_user = $this->ceh->select('COUNT(UserID) CountUser')
                                    ->where('UserID', $UserID)
                                    ->where("YARD_ID", $this->session->userdata("YARD_ID"))
                                    ->limit(1)
                                    ->get('SA_USERS');
    ...
    public function is_Admin() {
        if($this->session->userdata('GroupMenuID') == $this->_GroupAdmin) {
            return true;
        } else {
            return false;
        }
    ...
    public function access($method = '') {        
        $where = array(
            'p.GroupMenuID' => $this->session->userdata('GroupID')
        );
    ...
    public function allGroups() {
        $result = $this->ceh->where("YARD_ID", $this->session->userdata("YardID"))->get('SYS_PERMISSION');
    ...
    public function saveGroups($grID, $grName) {
        ...
        if($grID != ''){
            //update database
            $this->ceh->where('GroupMenuID', $grID)
                        ->where('YARD_ID', $item["YardID"])
                        ->limit(1)
    ...
    public function allUsers() {
        ...
        $this->ceh->where("YARD_ID", $this->session->userdata("YardID"));
    ...
    public function byUserGroupID($gId) {
        $stmt = $this->ceh->where('GroupID', $gId)
                            ->where('YARD_ID', $this->session->userdata("YardID"))
                            ->get('SA_USERS');
    ...
    public function byId($userId) {
        ...
        $this->ceh->where('u.YARD_ID', $this->session->userdata("YardID"));
        $this->ceh->order_by('u.UserID', 'ASC');
        $stmt = $this->ceh->get('SA_USERS AS u');
...
//        $munit = time() - 900;
//
//        $sql = "SELECT s.*, u.user_name, g.group_name FROM sessions s INNER JOIN SA_USERS u ON u.user_id = s.user_id INNER JOIN SYS_PERMISSION g ON u.group_id = g.group_id WHERE s.start_time > ? ORDER BY s.start_time DESC";
//
