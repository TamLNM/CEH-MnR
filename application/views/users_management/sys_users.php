				...
				<div class="col-md-12 col-sm-12 col-xs-12 table-responsive">
					<table id="contenttable" class="table table-striped display nowrap" cellspacing="0" style="width: 99.5%">
						...
						<tr>
							...
							<th col-name="YardID">YardID</th>
							<th col-name="GroupUserID">Nhóm người dùng</th>
							<th col-name="UserID">Mã người dùng</th>
							<th col-name="UserName">Tên người dùng</th>
							<th col-name="InpPwd">Mật khẩu</th>
							<th col-name="InpDefaultPwd">Mật khẩu mặc định</th>
							<th col-name="Pwd">Pwd</th>
							...
							<?php if(count($sysUsersList) > 0) {
								...
								<?php foreach($sysUsersList as $item){  ?>									
									<tr>
										...
										<td col-name="YardID"><?=$item['YardID'];?></td>
										<td col-name="GroupUserID"><?=$item['GroupUserID'];?></td>
										<td col-name="UserID"><?=$item['UserID'];?></td>
										<td col-name="UserName"><?=$item['UserName'];?></td>
										<td col-name="InpPwd">******</td>
										<td col-name="InDefaultPwd">******</td>
										<td col-name="Pwd"><?=$item['Pwd'];?></td>
				...
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					<div class="row form-group">
						...
						<div class="col-md-6 col-sm-6 col-xs-6 input-group">
                            <input class="form-control form-control-sm" id="beforePassword" placeholder="Mật khẩu cũ" type="password">                        
                            <span class="input-group-addon" id="beforePasswordIcon"><i class="la la-eye-slash"></i></i></span>
                        </div>
					</div>
					<div class="row form-group">
						<label class="col-md-5 col-sm-5 col-xs-5 col-form-label" style="text-align: right;">Mật khẩu mới (*)</label>
						<div class="col-md-6 col-sm-6 col-xs-6 input-group">
                            <input class="form-control form-control-sm" id="newPassword" placeholder="Mật khẩu cũ" type="password">                        
                            <span class="input-group-addon" id="newPasswordIcon"><i class="la la-eye-slash"></i></i></span>
                        </div>
					</div>
					<div class="row form-group">
						<label class="col-md-5 col-sm-5 col-xs-5 col-form-label" style="text-align: right;">Xác nhận mật khẩu (*)</label>
						<div class="col-md-6 col-sm-6 col-xs-6 input-group">
                            <input class="form-control form-control-sm" id="confirmNewPassword" placeholder="Xác nhận mật khẩu" type="password">                        
                            <span class="input-group-addon"  id="confirmNewPasswordIcon"><i class="la la-eye-slash"></i></i></span>
                        </div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	$(document).ready(function () {
		var _columns 			= ["STT", "rowguid", "YardID", "GroupUserID", "UserID", "UserName", "InpPwd", "InpDefaultPwd", "Pwd", "BirthDay", "Email", "IsActive"],
			tbl 	 			= $("#contenttable"),
		...
		<?php if(isset($sysGroupUsersList) && count($sysGroupUsersList) >= 0){?>
			sysGroupUsersList = <?=json_encode($sysGroupUsersList);?>;
		<?php } ?>
		...
		var dataTbl = tbl.newDataTable({
			...
			columnDefs: [
				...
				{ className: "text-center", targets: _columns.getIndexs(["GroupUserID", "UserID", "InpPwd", "InpDefaultPwd", "UserName", "BirthDay", "Email", "IsActive"])},
				{ className: "hiden-input", targets: _columns.getIndexs(["rowguid", "YardID", "Pwd"])},
			],
        ...
        function saveData(){        	
			...
			if(newData.length > 0){
				for (i = 0; i < newData.length; i++){
					newData[i]['InpPwd'] = xMD5(xMD5('CEH_hZWEzNzc45622NjdiOA==') + xMD5(newData[i]['InpPwd']));
					newData[i]['InpDefaultPwd'] = xMD5(xMD5('CEH_hZWEzNzc45622NjdiOA==') + xMD5(newData[i]['InpDefaultPwd']));
		...
		function postDel(data){
			var delData = data.map(p=>p[_columns.indexOf("rowguid")]);
		...
		$("#apply-change-password").on('click', function(){
			...
			if (xMD5(xMD5('CEH_hZWEzNzc45622NjdiOA==') + xMD5(beforePassword)) == data[_columns.getIndexs("Pwd")]){
				...
				var fChangePassword = {
					...
					'rowguid': data[_columns.getIndexs("rowguid")],
					'pass': newPassword,
				};
		...
		tbl.setExtendDropdown({
			target: "#cell-context",
			source: sysGroupUsersList,
			colIndex: _columns.indexOf("GroupUserID"), 
			onSelected: function(cell, value){ 
				var groupName = sysGroupUsersList.filter( p => p.GroupUserID == value).map( x => x.GroupUserName );
		...
		$('#beforePasswordIcon, #newPasswordIcon, #confirmNewPasswordIcon').on('click', function(){
   			var id 			= $(this).attr('id'),
   				getClass 	= $('#' + id + ' i').attr('class'),
   				idIconArr 	= '#beforePasswordIcon i, #newPasswordIcon i, #confirmNewPasswordIcon i',
   				idInputArr 	= '#beforePassword, #newPassword, #confirmNewPassword';

   			if (getClass == 'la la-eye-slash'){
   				$(idIconArr).removeClass();
    			$(idIconArr).addClass('la la-eye');
    			$(idInputArr).prop("type", "text");
   			}
   			else{
   				$(idIconArr).removeClass();
    			$(idIconArr).addClass('la la-eye-slash');
    			$(idInputArr).prop("type", "password");
   			}
	    });
	});
