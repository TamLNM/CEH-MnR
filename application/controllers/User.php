...
class User extends CI_Controller
{
    ...
    public function login() {
        ...
        $this->form_validation->set_rules('UserID', 'Tên đăng nhập', 'trim|required|alpha|min_length[3]|max_length[30]', array(
...
//        }
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(!empty($result)) {
                    if($result['IsActive'] == '0') {
                        ...
                        if ($result['UserGroupID'] == 'GroupAdmin'){
                            redirect(md5('home'));
                        }
                       
                        if ($result['UserGroupID'] == 'GroupTally'){
                            redirect(md5('tally'));
                        }
                        
                        if ($result['UserGroupID'] == 'GroupGateInspector'){
                            redirect(md5('gate_inspector'));
                        }
    ...
    private function createCaptcha($length = 6){
        ...
        $vals = array(
            'word'          => $streamkey,
            'img_path'      => $_SERVER["DOCUMENT_ROOT"] . '/ebilling/assets/img/captcha/',
            'img_url'       => base_url('assets/img/captcha/'),
            'font_path'     => $_SERVER["DOCUMENT_ROOT"] . '/ebilling/system/fonts/texb.ttf',
            'img_width'     => '150',
                'img_height'    => 37.7,
            'expiration'    => 7200,
            'font_size'     => 18,
    ...
    public function groups() {
        ...
        if($this->input->post('id')) {
            ...
            $this->funcs->udi_my("DELETE FROM groups WHERE group_id = ? AND YARD_ID = ?", array($id, $this->YardID));
            redirect(md5('user') . '/' . md5('groups'));
    ...
    public function users() {
        ...
        $this->data['GroupID'] = $this->input->post('GroupID') ? $this->input->post('GroupID') : '';
        ...
        if($this->data['type'] == 'grp'){
            $this->user->saveGroups($this->data['GroupID'], $this->data['groupname']);
    ...
    public function permission() {
        ...
        if($this->data['actions'] == 'searchUsers' && $this->data['GroupID']) {
            echo json_encode($this->user->byGroupID($this->data['GroupID']));
            exit;
        }

        if($this->data['actions'] == 'searchPermiss' && ($this->data['GroupID'] || $this->data['userid'])) {
            if($this->data['GroupID']) {
                $stmt = $this->ceh->where('GroupID', $this->data['GroupID'])->get('PERMISSION');
            } else {
        ...
        if($this->data['actions'] == 'acceptAllPermiss' && ($this->data['GroupID'] || $this->data['userid'])) {
            ...
            $per_detail = '';
            if($this->data['GroupID'] && empty($this->data['userid'])) {
                $this->ceh->where('GroupID', $this->data['GroupID']);
                $this->ceh->delete('PERMISSION');
        ...
        if($this->data['actions'] == 'selectpermiss' && ($this->data['GroupID'] || $this->data['userid'])) {
        ...
        if($this->data['actions'] == 'acceptPermiss' && ($this->data['GroupID'] || $this->data['userid'])) {
    ...
    private function remove_eir_seq(){
        $arrckname = array_keys($_COOKIE);
        foreach ($arrckname as $k => $v ) {
            unset($_COOKIE[$v]);
//            $pattern = '/(.*)eirno'.session_id().'/';
//            if(preg_match($pattern, $v, $r)){
//                if(isset($r)){
//                    $eir = $_COOKIE[$v];
//                    unset($_COOKIE[$v]);
//                    $eirqueue = $this->config->item('EIR_NO_QUEUE');
//                    str_replace(",".$eir, "", $eirqueue);
//
//                    $file_path = APPPATH.'config/config_site.php';
//                    if(file_exists($file_path))
//                    {
//                        $content = file_get_contents($file_path);
//                        $content = preg_replace("/.*EIR_NO_QUEUE.*=.*;/","$"."config['EIR_NO_QUEUE'] = '".implode(',', $eirqueue)."';", $content);
