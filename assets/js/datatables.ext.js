...
$.fn.extend({
	initComplete: function(x){
		...
		if (x && x.FILEIMPORT) {
			var cusImportFile = "<a class=\"btn\" href=\"<?=base_url('assets/excels/quote_and_repair.xlsx');?>\"><i style=\"color: #365899;\"><u>Tải file import mẫu...</u></i></a>";
			$(tbl.selector + '_wrapper .datatable-info-right').append(cusImportFile);
		}
		if (x && x.IMPORT) {
			var cusImport = "<label style=\"margin-right: 2px;\" for=\"fileinput\" class=\"btn btn-outline-info btn-sm\"><span class=\"btn-icon\"><i class=\"ti-import\"></i>Import</span></label><input id=\"fileinput\" name=\"fileinput\" style=\"visibility: hidden; width: 1px\" type=\"file\">";
			$(tbl.selector + '_wrapper .datatable-info-right').append(cusImport);
		}
		if (x && x.REFRESH) {
		...
		if (x && x.ADD) {
			var cusAdd = "<button id=\"" + cusName + "_btnAdd\" class=\"btn btn-outline-success btn-sm mr-1\" title=\"Thêm\"> \
				<span class=\"btn-icon\"><i class=\"fa fa-plus\"></i>Thêm</span></button>";
			// $(tbl.selector + '_wrapper .datatable-info-right').css({"margin-right":"0px","padding-top":"3px"}).append(cusAdd);
		...
		if (x && x.SAVE) {
			var cusSave = "<button id=\"" + cusName + "_btnSave\" class=\"btn btn-outline-primary btn-sm mr-1\" data-loading-text=\"<i class='la la-spinner spinner'></i>Lưu\" title=\"Lưu dữ liệu\"> \
				<span class=\"btn-icon\"><i class=\"fa fa-save\"></i>Lưu</span></button>";
			// $(tbl.selector + '_wrapper .datatable-info-right').css({"margin-right":"0px","padding-top":"3px"}).append(cusSave);
			$(tbl.selector + '_wrapper .datatable-info-right').append(cusSave);
		}
		if (x && x.DELETE) {
			var cusDelete = "<button id=\"" + cusName + "_btnDelete\" class=\"btn btn-outline-danger btn-sm mr-1\" data-loading-text=\"<i class='la la-spinner spinner'></i>Xóa\" title=\"Xóa\"> \
				<span class=\"btn-icon\"><i class=\"fa fa-trash\"></i>Xóa</span></button>";
			// $(tbl.selector + '_wrapper .datatable-info-right').css({"margin-right":"0px","padding-top":"3px"}).append(cusDelete);
	...
	newDataTable: function(opt){
		...
		if(opt){
			...
			if (opt.configColumns && opt.configColumns.length > 0){
				var icolumnDefs = [];
				for (var i=0; i< opt.configColumns.length; i++) {
					var g = opt.configColumns[i];
					// if ((g['Class_Name']+"").includes("hiden-input")) {
					// 	var xcolumnDefs = {
					// 		className: "",
					// 		targets: i
					// 	};
					...
					// else {
					// 	var xcolumnDefs = {
					// 		type: (g['C_Type'] ? g['C_Type'] : ""),
					// 		className: (g['Class_Name'] ? g['Class_Name'] : ""),
					// 		targets: i
					// 	};
					// }
					var xcolumnDefs = {
					...
					icolumnDefs.push(xcolumnDefs);
				}
				opt.columnDefs = icolumnDefs;
			}
			...
			if(datatypes != null && datatypes[1]){
				switch (datatypes[1]){
					...
					case "numeric":
						...
						break;
					case "checkbox":
						cell_data = '<label class="checkbox checkbox-primary"><input id="isActiveChb" type="checkbox" checked="true"><span class="input-span"></span></label>';
						break;
					case "color":
						cell_data = '<input type="color" value="#000000">';
						break;
	...
	updateSTT: function (col_index){
		...
		$.each(data.toArray(), function(idx, item){
			tbl.cell(idx, col_index).data(idx + 1).draw(false);
		});
	},
	...
	getRowFocusData: function(_saveColumns){
		return this.getChangedData(_saveColumns, ".x-row-selected");
	// 	var tbl = $(this).DataTable();
	// 	var rowIdx = $(this).find('td.focus').closest('tr').index();
	// 	var headers = $(this).find('thead:first tr:first');
	// 	var rowData = {};
	// 	var row = tbl.rows(rowIdx).data().toArray();
	// 	console.log(row);
	// 	$.each(row, function(i, t){
	// 		var colName = headers.find('th:eq('+i+')').first().attr('col-name');
	// 		if(colName == "STT"){
	...
	confirmDelete_News: function(callback){
		var tbl = $(this).DataTable();
		var headers = $(this).find('thead:first tr:first');
		if(tbl.rows('.selected').data().length == 0) return false;
        $.confirm({
            ...
            buttons: {
                ok: {
                    ...
                    action: function(){
						...
						console.log(result);
						if (delrow.length > 0) {
							$.each(delrow, function(idx, row){ 
								var rowData = {};
								$.each(row, function(i, t){
									var colName = headers.find('th:eq('+i+')').first().attr('col-name');
									if(colName == "STT") { return; }
									var content = t;
